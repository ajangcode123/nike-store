...
export default function Admin() {
  ...
  const handleAddProduct = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setProductId(product.id); // Simpan product ID untuk input stok
        setShowProductModal(false); // Tutup modal produk
        setShowStockModal(true); // Buka modal stok
  ...
  const handleEditProduct = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Update produk di state products tanpa perlu refresh
        setProducts(
        ...
        // Reset editProduct dan tutup modal
        setEditProduct(null);
  ...
  if (jwtDecode(Cookies.get("token")).role !== "admin") {
    navigate("/");
  } else
    return (
      <div>
        ...
        <div className="p-6 bg-blue-50 min-h-screen">
          ...
          {/* Add Admin Button */}
          <Link to="/register">
            <button className="ml-4 bg-gradient-to-r from-sky-400 to-blue-500 text-white px-6 py-2 rounded mt-6 shadow-lg hover:shadow-2xl transition-shadow duration-300">
              Add Admin Account
            </button>
          </Link>
        ...
        {showStockModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
            <div className="bg-white p-8 rounded shadow-lg">
              <h3 className="text-xl font-semibold">Add Sizes and Stock</h3>
              <div className="mt-4">
        ...
        {showEditStockModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
            <div className="bg-white p-8 rounded shadow-lg">
              <h3 className="text-xl font-semibold">Edit Sizes and Stock</h3>
              <div className="mt-4">
