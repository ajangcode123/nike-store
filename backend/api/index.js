...
app.put("/api/orders/:id", authenticateToken, async (req, res) => {
  try {
    await pool.query(
      "UPDATE orders SET order_date = $1, status = $2, total_amount = $3, shipping_address = $4 WHERE id = $5 AND user_id = $6",
      [req.body.order_date, req.body.status, req.body.total_amount, req.body.shipping_address, req.params.id, req.user.id]
    );
...
app.put("/api/reviews/:id", authenticateToken, async (req, res) => {
  try {
    await pool.query(
      ...
      [req.body.rating, req.body.comment, req.params.id, req.user.id]
    );
...
app.put("/api/payment/:id", authenticateToken, async (req, res) => {
  try {
    await pool.query(
      "UPDATE payment SET payment_method = $1, payment_status = $2 WHERE id = $3",
      [req.body.payment_method, req.body.payment_status, req.params.id]
    );
...
app.delete("/api/payment/:id", authenticateToken, async (req, res) => {
  try {
...
//     res.json({ success: true, message: "Order berhasil diproses" });
//   } catch (error) {
